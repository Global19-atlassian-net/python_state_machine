...
class BaseAdaptor(object):
    ...
    def get_potential_state_machine_attributes(self, clazz, oftype):
        l = []
        for key in clazz.__dict__:
            attr = clazz.__dict__[key]
            t = type(attr)
            if t==oftype:
                l.append((key, attr))
        return l
    ...
    def process_states(self, original_class):
        initial_state = None
        is_method_dict = dict()
        for member, value in self.get_potential_state_machine_attributes(original_class, State):
    ...
    def process_events(self, original_class):
        ...
        for member, value in self.get_potential_state_machine_attributes(original_class, Event):
